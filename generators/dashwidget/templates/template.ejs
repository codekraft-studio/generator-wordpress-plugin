 ...
 *
 */
class <%= childClassName %>_Dashwidget {
   ...
   */
	 ...
	 */
	protected $description = 'This is the <%= childClassName %> dashboard widget description.';

  public function __construct() {

		// Add widget options
		$this->update_dashboard_widget_options(
			$this->id, // The widget unique id
			array(), // Add any key/value options for the widget
			true //Add only (will not update existing options)
		);

		// Register widget
		wp_add_dashboard_widget( $this->id, __($this->title, '<%= projectName %>'), array($this, 'render_widget'), array($this, 'widget_configuration') );
	 ...
	 */
	public function render_widget($post, $callback_args) {
